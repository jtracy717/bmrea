...
$(function() {	
	...
	$("#doform").click(function() {
		...
		if(!/^\d{2}\/\d{2}\/\d{4}$/.test($("#sale_date").val())) {
			flag('#sale_date','Please specify a desired sale date');									
			ok = false;
		}

		if(!/^\d{4}$/.test($("#year").val())) {
			var year = $("#year").val();
			if((year<1600) || (year > 2011)) {
				flag('#year','Please estimate the year the home was built');									
				ok = false;
			}
		}
		...
		if(!$("#reason").val()) {
			flag('#reason',"Please let us know why you are selling");									
			ok = false;
		}
		...
		if(!$("#address").val()) {
			flag('#address',"Please enter the property street address");									
			ok = false;
		}							
